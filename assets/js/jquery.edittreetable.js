...
(function ($) {
  $.fn.bstreetable = function (options) {
    ...
    var settings = {
      ...
      text: {
        NodeDeleteText: "Bạn có muốn xóa không?",
      },
    };
    ...
    element.delegate(".j-remove", "click", function (event) {
      ...
      if (isRemoveAble) {
        ...
        if (settings.customconfirm(settings.text.NodeDeleteText)) {
          ...
          settings.noderemoveCallback(
            that.parents(".class-level-ul").attr("data-id"),
            function () {
    ...
    element.delegate(".j-addChild", "click", function () {
      ...
      if (!hasError) {
        ...
        var curLevel =
          $(this).parents(".class-level-ul").attr("data-level") - 0 + 1;
    ...
    element.delegate(".form-control", "blur", function () {
      ...
      data.pinnercode = curElement
        .parents(".class-level-" + (parentUl.attr("data-level") - 1))
        .children("ul")
        .attr("data-innercode");
        ...
        settings.nodeaddCallback(data, function (_data) {
          if (_data) {
            ...
            curElement
              .parent()
              .parent()
              .attr("data-innercode", _data.innercode);
    ...
    function renderHeader(_dom_header) {
      var dom_row = $("<div></div>");
      dom_row.append(
        $("<span class='maintitle'></span>").text(settings.maintitle)
      );
      ...
      switch (extfield.type) {
        case "input":
          generatedCol = $(
            ...
            .val(row[extfield.key])
            .attr("data-oldval", row[extfield.key])
            .attr("name", extfield.key);
          break;
        default:
          generatedCol = $("<span></span>").text(row[extfield.key]);
          break;
      }
      return generatedCol;
    }
    ...
    function generateTreeNode(curElement, row, curLevel, isPrepend) {
      ...
      if (curLevel - 0 >= settings.maxlevel) {
        $('<li class="j-expend"></li>')
          .append('<label class="fa p-xs"></label>')
          .append(
            $(
              "<input type='text' class='form-control input-sm' required placeholder='Nội dung bài học...' />"
            )
      ...
      if (settings.nodeaddEnable) {
        if (curLevel - 0 >= settings.maxlevel) {
          ...
          $("<li></li>")
            .append(
              $(
                '<button class="btn btn-outline btn-sm j-addChild"><i class="fa fa-plus"></i>' +
                  language.addchild +
                  "</button>"
              ).attr("data-id", row.id)
            )
